...
class MrpProductionSerialMatrix(models.TransientModel):
    ...
    def button_validate(self):
        ...
        if current_mo.product_qty > 1:
            mos = current_mo._split_productions(
                {current_mo: [1 for i in self.finished_lot_ids]}
            )
        else:
            mos = [current_mo]
        ...
        for index, finished_lot in enumerate(self.finished_lot_ids):
            mo = mos[index]
            mo.write({"lot_producing_id": finished_lot.id, "qty_producing": 1.0})
            mo.action_confirm()
            for move in mo.move_raw_ids + mo.move_byproduct_ids:
                matrix_line = self.line_ids.filtered(
                    lambda l: (
                        l.finished_lot_id == mo.lot_producing_id
                ...
                if matrix_line:
                    ...
                    move._update_reserved_quantity(
                        move.product_uom_qty, move.product_uom_qty, move.location_id
                    )
